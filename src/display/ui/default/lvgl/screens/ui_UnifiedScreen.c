...
void ui_UnifiedScreen_screen_init(void) {
    ...
    // --- Outer arc (temp in brew, temp in water/steam) ---
    ...
    // --- Inner arc (pressure in brew mode, hidden otherwise) ---
    ...
    lv_obj_set_size(center_stack, 200, 420);
    ...
    // --- Flush button (84px circle, blue tinted) ---
    ...
    lv_obj_set_style_text_font(ui_UnifiedScreen_tempLabel, &lv_font_montserrat_34, LV_PART_MAIN);
    ...
    lv_obj_set_style_text_font(ui_UnifiedScreen_pressureLabel, &lv_font_montserrat_34, LV_PART_MAIN);
...
void ui_UnifiedScreen_set_idle(void) {
    ...
    // Restore flush button idle style (blue circle)
    lv_obj_set_size(ui_UnifiedScreen_flushBtn, UI_FLUSH_BTN_SIZE, UI_FLUSH_BTN_SIZE);
    lv_obj_set_style_bg_color(ui_UnifiedScreen_flushBtn, UI_COLOR_BLUE, LV_PART_MAIN);
    lv_obj_set_style_bg_opa(ui_UnifiedScreen_flushBtn, UI_OPA(15), LV_PART_MAIN);
    lv_obj_set_style_border_color(ui_UnifiedScreen_flushBtn, UI_COLOR_BLUE, LV_PART_MAIN);
...
void ui_UnifiedScreen_set_mode_brew(void) {
    ...
    // Inner arc = blue (pressure), visible
    lv_obj_clear_flag(ui_UnifiedScreen_innerArc, LV_OBJ_FLAG_HIDDEN);
    lv_obj_set_style_arc_color(ui_UnifiedScreen_innerArc, UI_COLOR_BLUE, LV_PART_INDICATOR);
    lv_arc_set_range(ui_UnifiedScreen_innerArc, UI_PRESSURE_MIN * 10, UI_PRESSURE_MAX * 10);
    lv_arc_set_value(ui_UnifiedScreen_innerArc, 0);

    // Action btn = green play (restore clickable in case coming from steam)
    lv_obj_clear_flag(ui_UnifiedScreen_actionBtn, LV_OBJ_FLAG_HIDDEN);
...
void ui_UnifiedScreen_set_mode_water(void) {
    ...
    // Action btn = blue droplet (restore clickable in case coming from steam)
    lv_obj_clear_flag(ui_UnifiedScreen_actionBtn, LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_flag(ui_UnifiedScreen_actionBtn, LV_OBJ_FLAG_CLICKABLE);
    ...
    lv_obj_set_style_pad_top(ui_UnifiedScreen_actionBtn, 40, LV_PART_MAIN);
...
void ui_UnifiedScreen_set_mode_steam(void) {
    ...
    // Add spacing above action button for steam mode
    lv_obj_set_style_pad_top(ui_UnifiedScreen_actionBtn, 40, LV_PART_MAIN);
